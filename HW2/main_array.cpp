...
Employee* delete_employee(Employee* employees,int& employee_num, int id,Employee* &deleted_employees, int& deleted_employee_num){
    ...
    if (index != -1){
        Employee* new_deleted_employees = new Employee[deleted_employee_num + 1];
        for (int i = 0; i < deleted_employee_num; i++){
            new_deleted_employees[i].set_all(deleted_employees[i].get_id(),deleted_employees[i].get_salary(),deleted_employees[i].get_department());
        }
        new_deleted_employees[deleted_employee_num].set_all(employees[index].get_id(),employees[index].get_salary(),employees[index].get_department());
        deleted_employee_num++;
        delete[] deleted_employees;
        deleted_employees = new_deleted_employees;
...
int main(int argc, char** argv) {
    ...
    Employee* deleted_employees = NULL;
