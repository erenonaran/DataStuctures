...
Employee* add_employee(Employee* employees,int& employee_num, int salary, int department){
    clock_t start = clock();
    ...
    employees_extended[employee_num].set_all(max_id+1,salary,department);
    employee_num++;
    delete[] employees;
    clock_t end = clock();
...
Employee* delete_employee(Employee* employees,int& employee_num, int id){
    ...
    if (index != -1){
        ...
        cerr << "ERROR: Invalid ID to delete" << endl;
        clock_t end = clock();
        cout << "DELETE:  " <<(double)(end - start) * 1000 / CLOCKS_PER_SEC << endl;
        return employees;
    }
...
void update_employee(Employee* employees, int employee_num, int id, int salary, int department){
    ...
    if (index != -1) {
        ...
        cerr << "ERROR: Invalid ID to update" << endl;
    }
    clock_t end = clock();
    ...
    while (getline(text_file,line)){
        if (!is_empty_line(line)){
            ...
            if (operation == "ADD"){
                getline(line_stream, employee_salary,';');
                getline(line_stream, employee_department,'\n');
                employees = add_employee(employees,employee_num,stoi(employee_salary),stoi(employee_department));
                ...
                getline(line_stream, employee_id,'\n');
                employees = delete_employee(employees,employee_num,stoi(employee_id));
            } else {
