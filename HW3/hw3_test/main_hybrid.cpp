...
BinaryTreeVector::~BinaryTreeVector(){
    for (auto& BST: this->BSTVector){
        delete BST;
    }
}

void BinaryTreeVector::insert(Employee* employee){
    int id = employee->get_id();
    while (static_cast<size_t>(id / ARR_SIZE) >= this->BSTVector.size()){
        this->BSTVector.push_back(new BinaryTree);
    }
    this->BSTVector[(id / ARR_SIZE)]->insert(employee);
}

void BinaryTreeVector::insert(int salary, int department){

    if (this->BSTVector.empty()){
        BinaryTree* first_tree = new BinaryTree;
        this->BSTVector.push_back(first_tree);
        first_tree->insert(new Employee(0,salary,department));
    } else {
        Node* temp = this->BSTVector.back()->get_root();
        while(temp->get_right() != nullptr){
            temp = temp->get_right();
        }
        int id = temp->get_employee()->get_id() + 1;
        if (id % ARR_SIZE == 0){
            BinaryTree* new_tree = new BinaryTree;
            this->BSTVector.push_back(new_tree);
            new_tree->set_root(new Node(new Employee(id,salary,department)));
        } else {
            temp->set_right(new Node(new Employee(id,salary,department)));
        }
    }
}

void BinaryTreeVector::update(int id, int salary, int department){
    if (static_cast<size_t>(id / ARR_SIZE) >= this->BSTVector.size()){
        cerr << "ERROR: An invalid ID to update" << endl;
        return;
    }
    this->BSTVector[(id / ARR_SIZE)]->update(id,salary,department);
}

void BinaryTreeVector::remove(int id){
    if (static_cast<size_t>(id / ARR_SIZE) >= this->BSTVector.size()){
        cerr << "ERROR: An invalid ID to delete" << endl;
        return;
    }
    this->BSTVector[(id / ARR_SIZE)]->remove(id);
    while (this->BSTVector.back()->get_root() == nullptr){
        delete this->BSTVector.back();
        this->BSTVector.pop_back();
    }
}

double BinaryTreeVector::getHeight(){
    double total = 0;
    for (auto& BST : this->BSTVector){
        total += BST->getHeight();
    }
    return total / this->BSTVector.size();
}

void BinaryTreeVector::printToFile(ofstream& output_file){
...
int main(int argc, char** argv) {
    ...
    BinaryTreeVector EmployeeTreeVector;
    ...
    while(getline(dataset_file,line)){
        if(!is_empty_line(line)){
            ...
            EmployeeTreeVector.insert(employee);
        }
    }
    dataset_file.close();
    ...
    while (getline(op_file,line)){
        if (!is_empty_line(line)){
            ...
            if (operation == "ADD"){
                ...
                getline(line_stream,department_string,'\n');
                //clock_t start = clock();
                EmployeeTreeVector.insert(stoi(salary_string),stoi(department_string));
                //clock_t end = clock();
                ...
                getline(line_stream,department_string,'\n');
                //clock_t start = clock();
                EmployeeTreeVector.update(stoi(id_string),stoi(salary_string),stoi(department_string));
                //clock_t end = clock();
                ...
                getline(line_stream,id_string,'\n');
                //clock_t start = clock();
                EmployeeTreeVector.remove(stoi(id_string));
                //clock_t end = clock();
                //cout << "DELETE: "<< (double)(end - start) * 1000 / CLOCKS_PER_SEC << endl;
            } else if (operation == "HEIGHT"){
                cout << "H "<< EmployeeTreeVector.getHeight() << endl;
            } else {
    ...
    EmployeeTreeVector.printToFile(output_file);
