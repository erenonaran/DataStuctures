...
void BinaryTree::insert(int salary, int department){
    int max_id = this->max_id_node->get_employee()->get_id();
    Node* new_employee = new Node(new Employee(max_id+1,salary,department));
    max_id_node->set_right(new_employee);
    max_id_node = new_employee;
}
...
void BinaryTree::remove(int id){
    ...
    if (id == max_id_node->get_employee()->get_id()){
        if (this->root == nullptr){
            this->max_id_node = nullptr;
        } else {
            Node* temp = this->root;
